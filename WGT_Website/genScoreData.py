...
def getRoundIDs (tourn_id):            
    ...
    for each in rounds:
        if each.round_day == 'Sat':
            round1_id = each.round_id
        else:
            round2_id = each.round_id
    
    return round1_id, round2_id 
    
def readScores():
    import csv
    f = open("golferScores.csv", "r")
...
def getJSONstr():
    ...
    for score in scores:
        ...
        roundScores.append (json_grs)
         
    jsonStr = str (roundScores).replace ("'",'')
